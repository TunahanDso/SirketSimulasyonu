...
public sealed class YazilimBorsasiSunucusu : IAsyncDisposable
{
    ...
    public Task BaslatAsync(CancellationToken cancellationToken)
    {
        ...
        KonsolKayitcisi.Basari($"Yazılım borsası v5 yayında | Port: {_ayarlar.CanliPanoPortu}");
    ...
    private async Task IstemciyiYonetAsync(TcpClient istemci, CancellationToken cancellationToken)
    {
        using (istemci)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(ilk) || ilk.Length > 8192) { await CevapAsync(akis, 400, "Bad Request", "text/plain; charset=utf-8", "Geçersiz istek.", cancellationToken); return; }
                for (int i = 0; i < 100; i++) { string? h = await okuyucu.ReadLineAsync(cancellationToken); if (string.IsNullOrEmpty(h)) break; }
                string[] parcalar = ilk.Split(' ', 3, StringSplitOptions.RemoveEmptyEntries);
                if (parcalar.Length != 3 || !string.Equals(parcalar[0], "GET", StringComparison.OrdinalIgnoreCase)) { await CevapAsync(akis, 405, "Method Not Allowed", "text/plain; charset=utf-8", "Yalnız GET desteklenir.", cancellationToken); return; }
                string yol = parcalar[1].Split('?', 2)[0];
                switch (yol)
                {
                    case "/": case "/index.html": await CevapAsync(akis, 200, "OK", "text/html; charset=utf-8", YazilimBorsasiHtml.Icerik, cancellationToken); break;
                    case "/api/durum": await CevapAsync(akis, 200, "OK", "application/json; charset=utf-8", await DurumJsonuOlusturAsync(cancellationToken), cancellationToken); break;
                    case "/api/saglik": await CevapAsync(akis, 200, "OK", "application/json; charset=utf-8", JsonSerializer.Serialize(new { durum = "calisiyor", tickNumarasi = _tickGetir(), surum = "borsa-v5" }, JsonAyarlari), cancellationToken); break;
                    case "/favicon.ico": await CevapAsync(akis, 204, "No Content", "image/x-icon", string.Empty, cancellationToken); break;
                    default: await CevapAsync(akis, 404, "Not Found", "text/plain; charset=utf-8", "Sayfa bulunamadı.", cancellationToken); break;
                }
            }
    ...
    private async Task<string> DurumJsonuOlusturAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (SirketKaydi kayit in _sirketler.SirketKayitlari)
        {
            ...
            borsaSirketleri.Add(new JsonObject
            {
                ...
                ["kasa"] = kayit.Kasa, ["netGelir"] = kayit.NetGelir, ["toplamGelir"] = kayit.ToplamGelir,
                ["toplamCeza"] = kayit.ToplamCeza, ["toplamGuvenlikKaybi"] = kayit.ToplamGuvenlikKaybi,
            ...
            foreach (JsonObject teknik in teknikProtokoller.OfType<JsonObject>())
                protokolPiyasasi.Add(new JsonObject { ["sirketKimligi"] = kayit.SirketKimligi, ["sirketAdi"] = kayit.SirketAdi, ["teknikProtokol"] = teknik["protokol"]?.DeepClone(), ["gecerli"] = teknik["gecerli"]?.DeepClone(), ["piyasada"] = teknik["piyasada"]?.DeepClone(), ["piyasaProtokolKimligi"] = teknik["piyasaProtokolKimligi"]?.DeepClone() });
        ...
        {
            ["motor"] = JsonSerializer.SerializeToNode(new { _ayarlar.MotorKimligi, _ayarlar.ProtokolSurumu, tickNumarasi = tick, tickSuresiSaniye = _ayarlar.TickSuresiSaniye, katalogSurumu = _katalog.KatalogSurumu, baslangicZamani = _baslangic, sunucuZamani = DateTimeOffset.UtcNow, panoPortu = _ayarlar.CanliPanoPortu, yonetimPortu = _ayarlar.SirketYonetimPortu, borsaSurumu = "5.0" }, JsonAyarlari),
            ["genel"] = JsonSerializer.SerializeToNode(new
            {
                toplamSirketSayisi = _sirketler.SirketKayitlari.Count, bagliSirketSayisi = _sirketler.SirketKayitlari.Count(x => x.BagliMi),
                toplamSirketKasasi = _sirketler.SirketKayitlari.Sum(x => x.Kasa), toplamNetGelir = _sirketler.SirketKayitlari.Sum(x => x.NetGelir),
                toplamSirketDegeri = sirketListesi.Sum(x => Dec(x["isletim"]?["sirketDegeri"])), toplamBorc = sirketListesi.Sum(x => Dec(x["isletim"]?["toplamBorc"])),
                toplamAbone = sirketListesi.Sum(x => Int(x["isletim"]?["toplamAboneSayisi"])),
                teknikUygulamaSayisi = uygulamaPiyasasi.Count, piyasadakiUygulamaSayisi = uygulamaPiyasasi.OfType<JsonObject>().Count(x => Bool(x["piyasada"])),
                teknikProtokolSayisi = protokolPiyasasi.Count, piyasadakiProtokolSayisi = protokolPiyasasi.OfType<JsonObject>().Count(x => Bool(x["piyasada"])),
                toplamHizmetIlani = _sirketler.SirketKayitlari.Sum(x => x.Hizmetler.Count), toplamGuvenlikKaybi = _sirketler.SirketKayitlari.Sum(x => x.ToplamGuvenlikKaybi),
                toplamEngellenenSaldiri = _sirketler.SirketKayitlari.Sum(x => x.EngellenenSaldiriSayisi), toplamBasariliSaldiri = _sirketler.SirketKayitlari.Sum(x => x.BasariliSaldiriSayisi),
                toplamMusteriSayisi = _musteriler.Musteriler.Count, aktifMusteriSayisi = _musteriler.AktifMusteriSayisi,
                ...
                toplamMusteriBakiyesi = _musteriler.ToplamMusteriBakiyesi, toplamMusteriHarcamasi = _musteriler.ToplamMusteriHarcamasi
            }, JsonAyarlari),
            ["piyasa"] = JsonSerializer.SerializeToNode(piyasa, JsonAyarlari),
            ["sirketler"] = borsaSirketleri, ["hizmetPiyasasi"] = HizmetPiyasasiniOlustur(piyasa),
            ["uygulamaPiyasasi"] = uygulamaPiyasasi, ["protokolPiyasasi"] = protokolPiyasasi,
            ["isletimSistemiPazari"] = JsonSerializer.SerializeToNode(IsletimSistemiPazarDeposu.Getir(), JsonAyarlari),
            ["krediPiyasasi"] = krediPiyasasi, ["sozlesmePiyasasi"] = sozlesmePiyasasi, ["yatirimPiyasasi"] = yatirimPiyasasi,
            ["olaylar"] = JsonSerializer.SerializeToNode(KonsolKayitcisi.SonKayitlariGetir(300), JsonAyarlari)
        };
