...
public sealed class YazilimBorsasiSunucusu : IAsyncDisposable
{
    ...
    public Task BaslatAsync(CancellationToken cancellationToken)
    {
        ...
        KonsolKayitcisi.Basari($"Yazılım borsası V7 yayında | Port: {_ayarlar.CanliPanoPortu}");
        ...
        using (istemci)
        {
            ...
            try
            {
                ...
                switch (yol)
                {
                    ...
                    case "/api/saglik":
                        await CevapAsync(akis, 200, "OK", "application/json; charset=utf-8", JsonSerializer.Serialize(new { durum = "calisiyor", tickNumarasi = _tickGetir(), surum = "borsa-v7" }, JsonAyarlari), cancellationToken); break;
                    case "/favicon.ico":
