...
public sealed class YazilimBorsasiSunucusu : IAsyncDisposable
{
    ...
    public Task BaslatAsync(CancellationToken ct)
    {
        ...
        KonsolKayitcisi.Basari($"Yazılım borsası V9.2 yayında | Port: {_ayarlar.CanliPanoPortu}");
    ...
    private async Task IstemciAsync(TcpClient istemci, CancellationToken ct)
    {
        using (istemci)
        {
            try
            {
                ...
                if (p.Length < 2 || p[0] != "GET")
                {
                    await CevapAsync(akis, 405, "text/plain; charset=utf-8", "Yalnız GET.", ct); return;
                }
                string yol = p[1].Split('?', 2)[0];
                if (yol is "/" or "/index.html")
                    await CevapAsync(akis, 200, "text/html; charset=utf-8", YazilimBorsasiHtml.Icerik, ct);
                else if (yol == "/api/durum")
                    await CevapAsync(akis, 200, "application/json; charset=utf-8", await DurumAsync(ct), ct);
                else if (yol == "/api/saglik")
                    await CevapAsync(akis, 200, "application/json; charset=utf-8",
                        JsonSerializer.Serialize(new { durum = "calisiyor", surum = "borsa-v9.2", tick = _tick() }, JsonAyarlari), ct);
    ...
    private async Task<string> DurumAsync(CancellationToken ct)
    {
        ...
        Dictionary<string, V9SirketPazarOzeti> ozetler = pazar.SirketOzetleri
        ...
        foreach (SirketKaydi sirket in _sirketler.SirketKayitlari)
        {
            ...
            try
            {
                panel = JsonNode.Parse(await _isletim.PanelJsonuOlusturAsync(sirket.SirketKimligi, ct)) as JsonObject ?? new();
            }
            ...
            sirketler.Add(JsonSerializer.SerializeToNode(new
            {
                ...
                hizmetGeliri = ozet?.HizmetGeliri ?? 0,
                uygulamaGeliri = ozet?.UygulamaGeliri ?? 0,
                abonelikGeliri = ozet?.AbonelikGeliri ?? 0,
                protokolGeliri = ozet?.ProtokolGeliri ?? 0,
                olayGeliri = ozet?.OlayGeliri ?? 0,
                isletmeGideri = ozet?.IsletmeGideri ?? 0,
                finansmanGideri = ozet?.FinansmanGideri ?? 0,
                olayGideri = ozet?.OlayGideri ?? 0,
                toplamGelir,
                toplamGider,
                netKazanc = ozet?.NetKazanc ?? 0,
                borc,
                toplamKapasite = kapasite?.ToplamFizikselKapasite ?? 0,
                hizmetHavuzu = kapasite?.HizmetHavuzu ?? 0,
                urunHavuzu = kapasite?.UrunHavuzu ?? 0,
                ...
                tahminiHisseFiyati = Dec(isletim["tahminiHisseFiyati"])
            }, JsonAyarlari));
            ...
            foreach (JsonObject teknik in (kod["uygulamalar"] as JsonArray ?? []).OfType<JsonObject>())
            {
                ...
                uygulamalar.Add(new JsonObject
                {
                    ...
                    ["dogrulama"] = teknik["dogrulama"]?.DeepClone(),
                    ["piyasada"] = teknik["piyasada"]?.DeepClone(),
                    ["urun"] = urun?.DeepClone(),
            ...
            foreach (JsonObject p in (kod["protokoller"] as JsonArray ?? []).OfType<JsonObject>())
                protokoller.Add(new JsonObject
                {
                    ["sirketAdi"] = sirket.SirketAdi,
                    ["protokol"] = p["protokol"]?.DeepClone(),
                    ["piyasada"] = p["piyasada"]?.DeepClone()
                });
        }

        List<object> cv = MusteriCvleri(tick);
        List<object> hizmetPazari = pazar.HizmetTalepleri.Values
            .OrderByDescending(x => x.BuTickTalep)
            .Select(x => (object)new
            {
                x.Anahtar,
                x.Ad,
                ...
                x.Gecmis,
                x.ArzDagilimi,
                sabitFiyat = MotorHizmetFiyatlari.Fiyat(
                    x.Anahtar.Split('@')[0],
                    x.Anahtar.Contains('@') ? x.Anahtar.Split('@')[1] : "1.0")
            }).ToList();
        List<object> haberler = pazar.Haberler
            .OrderByDescending(x => x.TickNumarasi)
            .ThenByDescending(x => x.Zaman)
            .Take(200)
            .Cast<object>()
            .Concat(KonsolKayitcisi.SonKayitlariGetir(80).Select(x => (object)new
            {
                baslik = x.Seviye,
                aciklama = x.Mesaj,
                tickNumarasi = tick,
        ...
        var cevap = new
        {
            motor = new
            {
                ...
                musteriSayisi = _musteriler.Musteriler.Count,
                katalogHizmetSayisi = _katalog.Hizmetler.Count,
                borsaSurumu = "9.2"
            },
            genel = new
            {
                toplamSirket = _sirketler.SirketKayitlari.Count,
                bagliSirket = _sirketler.SirketKayitlari.Count(x => x.BagliMi),
                toplamKasa = _sirketler.SirketKayitlari.Sum(x => x.Kasa),
                aktifMusteri = _musteriler.AktifMusteriSayisi,
                ...
                osTalebi = pazar.IsletimSistemiTalebi.BuTickTalep,
                osKarsilanan = pazar.IsletimSistemiTalebi.KarsilananTalep,
                ...
                toplamProtokolGeliri = pazar.SirketOzetleri.Sum(x => x.ProtokolGeliri),
                toplamOlayGeliri = pazar.SirketOzetleri.Sum(x => x.OlayGeliri),
                toplamGelir = pazar.SirketOzetleri.Sum(x => x.HizmetGeliri + x.UygulamaGeliri + x.ProtokolGeliri + x.OlayGeliri),
                toplamGider = pazar.SirketOzetleri.Sum(x => x.IsletmeGideri + x.FinansmanGideri + x.OlayGideri),
                toplamNet = pazar.SirketOzetleri.Sum(x => x.NetKazanc),
                toplamSirketDegeri = _sirketler.SirketKayitlari.Sum(s =>
                    sirketler.OfType<JsonObject>().FirstOrDefault(x => Str(x["sirketKimligi"]).Equals(s.SirketKimligi, StringComparison.OrdinalIgnoreCase)) is JsonObject n
                        ? Dec(n["sirketDegeri"])
                        : 0)
            },
            sirketler,
    ...
    private List<object> MusteriCvleri(long tick)
    {
        ...
        for (int i = 0; i < Math.Min(200, aktif.Count); i++)
        {
            ...
            sonuc.Add(new
            {
                ...
                m.GelirSegmenti,
                m.Bakiye,
                m.IsletimSistemiKimligi,
                m.IsletimSistemiSirketKimligi,
                uygulamalar = m.KullandigiUygulamalar,
                m.ToplamHarcama,
                m.BasariliIsSayisi,
                m.BasarisizIsSayisi,
                m.TercihEdilenSirketKimligi,
                m.IsletimSistemiMemnuniyeti,
        ...
        CancellationToken ct)
    {
        ...
        await akis.WriteAsync(Encoding.ASCII.GetBytes(h), ct);
