...
public sealed class IsYoneticisi
{
    ...
    private readonly Random _rastgele = new(20260725);
    ...
    {
        IsIslemeOzeti ozet = new() { TickNumarasi = tick, ToplamTalepSayisi = talepler.Count, KotuNiyetliIsSayisi = talepler.Count(x => x.KotuNiyetli) };
        foreach (HizmetTalebi talep in talepler)
        {
            ct.ThrowIfCancellationRequested();
            Ozetle(ozet, await TalebiIsleAsync(tick, talep, ct));
        }
        KonsolKayitcisi.Bilgi($"V9 iş özeti | Örnek {ozet.ToplamTalepSayisi:N0} | Başarılı {ozet.BasariliIsSayisi:N0} | Başarısız {ozet.BasarisizIsSayisi:N0} | Yarım dahil ciro {ozet.ToplamCiro:N2} TL");
        return ozet;
    }
    ...
    public async Task<IsAtamaSonucu> TalebiIsleAsync(long tick, HizmetTalebi talep, CancellationToken ct)
    {
        ...
        if (musteri is null || !musteri.Aktif) return IsAtamaSonucu.BasarisizSonuc(talep.IsKimligi, null, "Müşteri aktif değil.");
        decimal butce = Math.Min(talep.AzamiButce, musteri.Bakiye);
        IReadOnlyList<SirketAdayi> adaylar = Adaylar(musteri, talep, butce);
        if (adaylar.Count == 0)
        {
            _musteriler.IslemSonucunuKaydet(talep, false, 0, 0, "Piyasada uygun arz yok.");
            return IsAtamaSonucu.BasarisizSonuc(talep.IsKimligi, null, "Hizmeti karşılayan şirket bulunamadı.");
        }
        ...
        {
            MesajTuru = MesajTurleri.IsIstegi, IstekKimligi = $"istek-{Guid.NewGuid():N}", IsKimligi = talep.IsKimligi,
            TickNumarasi = tick, MusteriKimligi = talep.MusteriKimligi, HizmetKimligi = talep.HizmetKimligi,
            HizmetSurumu = talep.HizmetSurumu, TeklifEdilenTutar = ucret, ZamanAsimiMs = zamanAsimi,
            IstekVerisiJson = talep.IstekVerisiJson, OlusturulmaZamani = DateTimeOffset.UtcNow
        };

        try
        {
            IsSonucuMesaji cevap = await aday.Baglanti.IsIstegiGonderVeSonucuBekleAsync(istek, ct);
            if (talep.KotuNiyetli) return SiberSonuc(talep, sirket, cevap, zamanAsimi);
            ...
            if (!dogrulama.Gecerli)
            {
                if (MotorKusuru(talep, cevap, dogrulama))
                {
                    ...
                    return IsAtamaSonucu.BasarisizSonuc(talep.IsKimligi, sirket.SirketKimligi, "Motor istek kusuru; şirket etkilenmedi.", cevap.IslemSuresiMs);
                }
            ...
            _musteriler.IslemSonucunuKaydet(talep, true, ucret, cevap.IslemSuresiMs, "Tam ödeme yapıldı.");
        ...
        {
            if (talep.KotuNiyetli) return SiberIhlal(talep, sirket, zamanAsimi, "Saldırı zaman aşımı oluşturdu.");
        ...
        {
            if (talep.KotuNiyetli) return SiberIhlal(talep, sirket, 0, "Saldırı sunucu hatasına yol açtı.");
            return Basarisiz(talep, sirket, ucret, 0, hata.Message, false, 1.25);
        }
    }

    private IsAtamaSonucu Basarisiz(HizmetTalebi talep, SirketKaydi sirket, decimal tam, double sure, string aciklama, bool zamanAsimi, double carpan)
    {
        decimal yarim = decimal.Round(tam * 0.5m, 2);
        decimal odeme = _musteriler.MusteridenOdemeAl(talep.MusteriKimligi, yarim) ? yarim : 0;
        if (odeme > 0) { sirket.Kasa += odeme; sirket.ToplamGelir += odeme; }
        sirket.IsIptalEt();
        ...
        return new IsAtamaSonucu { Basarili = false, IsKimligi = talep.IsKimligi, SirketKimligi = sirket.SirketKimligi, IslemTutari = odeme, IslemSuresiMs = Math.Max(0, sure), SonucAciklamasi = aciklama };
    }

    private IsAtamaSonucu SiberSonuc(HizmetTalebi talep, SirketKaydi sirket, IsSonucuMesaji cevap, int zamanAsimi)
    {
        if (!cevap.Basarili && new[] { "GUVENLIK_REDDI", "ISTEK_GUVENLI_DEGIL", "KOTU_NIYETLI_ISTEK" }.Contains(cevap.HataKodu ?? "", StringComparer.OrdinalIgnoreCase))
        {
        ...
        return SiberIhlal(talep, sirket, cevap.IslemSuresiMs, "Saldırı normal iş gibi çalıştırıldı.");
    }

    private IsAtamaSonucu SiberIhlal(HizmetTalebi talep, SirketKaydi sirket, double sure, string aciklama)
    {
        decimal kayip = Math.Min(Math.Max(0, sirket.Kasa), Math.Clamp(talep.OlasiGuvenlikKaybi, 10m, 5_000m));
        sirket.IsIptalEt();
        sirket.IptalEdilenIsSayisi = Math.Max(0, sirket.IptalEdilenIsSayisi - 1);
        sirket.BasarisizIsSayisi++; sirket.BasariliSaldiriSayisi++; sirket.Kasa -= kayip; sirket.ToplamGuvenlikKaybi += kayip;
        sirket.GuvenlikPuani = Math.Max(0, sirket.GuvenlikPuani - 0.18);
        sirket.GuvenilirlikPuani = Math.Max(0, sirket.GuvenilirlikPuani - 0.10);
        sirket.ItibarPuani = Math.Max(0, sirket.ItibarPuani - 0.07);
    ...
    private IReadOnlyList<SirketAdayi> Adaylar(Musteri musteri, HizmetTalebi talep, decimal butce)
    {
        ...
        foreach (SirketBaglantisi baglanti in _sirketler.BaglantilariGetir())
        {
            ...
            if (!baglanti.Bagli || h is null || !h.Aktif || h.BirimFiyat > butce || !s.HizmetIcinKapasiteVarMi(talep.HizmetKimligi, talep.HizmetSurumu)) continue;
            double sadakat = string.Equals(musteri.TercihEdilenSirketKimligi, s.SirketKimligi, StringComparison.OrdinalIgnoreCase) ? 100 : 50;
            double puan = s.KodKalitesiPuani * .28 + s.PerformansPuani * .19 + s.GuvenlikPuani * .18 + s.GuvenilirlikPuani * .15 + s.ItibarPuani * .08 + s.KapasitePuaniHesapla(h) * .07 + sadakat * .05;
            sonuc.Add(new SirketAdayi { Baglanti = baglanti, Sirket = s, Hizmet = h, FiyatPuani = 100, ItibarPuani = s.ItibarPuani, GuvenilirlikPuani = s.GuvenilirlikPuani, KodKalitesiPuani = s.KodKalitesiPuani, PerformansPuani = s.PerformansPuani, GuvenlikPuani = s.GuvenlikPuani, HizPuani = s.HizPuaniHesapla(), KapasitePuani = s.KapasitePuaniHesapla(h), SadakatPuani = sadakat, ToplamPuan = puan });
        }
    ...
    private SirketAdayi Sec(IReadOnlyList<SirketAdayi> adaylar, bool saldiri)
    {
        ...
        double Agirlik(SirketAdayi x) => saldiri ? Math.Max(1, 105 - x.GuvenlikPuani + (100 - x.KodKalitesiPuani) * .35) : Math.Max(1, x.ToplamPuan);
        double secim; lock (_rastgeleKilidi) secim = _rastgele.NextDouble() * adaylar.Sum(Agirlik);
        foreach (SirketAdayi a in adaylar) { secim -= Agirlik(a); if (secim <= 0) return a; }
        return adaylar[^1];
    }

    private static bool MotorKusuru(HizmetTalebi talep, IsSonucuMesaji cevap, SonucDogrulamaSonucu dogrulama)
    {
        if (cevap.Basarili || cevap.HataKodu is not ("ISTEK_VERISI_GECERSIZ" or "GECERSIZ_ISTEK" or "EKSIK_ALAN")) return false;
        Match m = ZorunluAlan.Match(dogrulama.Aciklama ?? "");
        if (!m.Success) return false;
        try
        {
            using JsonDocument d = JsonDocument.Parse(talep.IstekVerisiJson);
            return !d.RootElement.TryGetProperty(m.Groups["alan"].Value, out JsonElement v) || v.ValueKind == JsonValueKind.Null || v.ValueKind == JsonValueKind.String && string.IsNullOrWhiteSpace(v.GetString());
        }
        catch { return true; }
    }
    ...
    private static void Ozetle(IsIslemeOzeti ozet, IsAtamaSonucu sonuc)
    {
        ...
        {
            if (sonuc.SaldiriEngellendi) ozet.EngellenenSaldiriSayisi++; else { ozet.BasariliSaldiriSayisi++; ozet.ToplamGuvenlikKaybi += sonuc.GuvenlikKaybi; }
            ozet.BasarisizIsSayisi++;
        }
        else if (sonuc.Basarili) ozet.BasariliIsSayisi++; else ozet.BasarisizIsSayisi++;
