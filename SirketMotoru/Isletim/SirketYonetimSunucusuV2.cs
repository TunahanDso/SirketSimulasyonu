...
public sealed class SirketYonetimSunucusu : IAsyncDisposable
{
    ...
    public SirketYonetimSunucusu(MotorAyarlari ayarlar, KodTabanliSirketIsletimYoneticisi isletim)
    {
    ...
    public Task BaslatAsync(CancellationToken cancellationToken)
    {
        ...
        KonsolKayitcisi.Basari($"Şirket yönetim merkezi v3 yayında | Port: {_ayarlar.SirketYonetimPortu}");
    ...
    private async Task YonlendirAsync(NetworkStream akis, HttpIstegi istek, CancellationToken cancellationToken)
    {
        ...
        {
            await JsonCevabiAsync(akis, 200, new { durum = "calisiyor", surum = "yonetim-v3", port = _ayarlar.SirketYonetimPortu, aktifOturum = _oturumlar.Count }, null, cancellationToken);
            return;
        }
        ...
        {
            string json = await _isletim.PanelJsonuOlusturAsync(oturum.SirketKimligi, cancellationToken);
            await CevapAsync(akis, 200, "application/json; charset=utf-8", json, null, cancellationToken);
            return;
        }
        ...
        int kod = yol.StartsWith("/api/", StringComparison.Ordinal) ? (sonucIslem.Basarili ? 200 : 400) : 404;
    ...
    {
        try { return JsonSerializer.Deserialize<T>(json, JsonAyarlari); } catch { return null; }
    }

    private static string SirketKimliginiOku(object? veri)
    {
        if (veri is null) return string.Empty;
        try { return JsonSerializer.SerializeToNode(veri, JsonAyarlari)?["sirketKimligi"]?.GetValue<string>() ?? string.Empty; } catch { return string.Empty; }
    }
    ...
    private static async Task CevapAsync(NetworkStream akis, int kod, string tur, string govde, Dictionary<string, string>? ek, CancellationToken cancellationToken)
    {
        ...
        string durum = kod switch { 200 => "OK", 204 => "No Content", 400 => "Bad Request", 401 => "Unauthorized", 404 => "Not Found", 405 => "Method Not Allowed", _ => "Error" };
        StringBuilder b = new();
