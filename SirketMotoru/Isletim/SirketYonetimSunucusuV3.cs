...
public sealed class SirketYonetimSunucusu : IAsyncDisposable
{
    ...
    public SirketYonetimSunucusu(MotorAyarlari ayarlar, KodTabanliSirketIsletimYoneticisi isletim, EkosistemYoneticisi ekosistem)
    {
    ...
    public Task BaslatAsync(CancellationToken cancellationToken)
    {
        ...
        _baslatildi = true;
        if (!_ayarlar.SirketYonetimAktif) { KonsolKayitcisi.Bilgi("Şirket yönetim kapısı kapalı."); return Task.CompletedTask; }
        ...
        KonsolKayitcisi.Basari($"Şirket yönetim merkezi v5 yayında | Port: {_ayarlar.SirketYonetimPortu}");
    ...
    private async Task IstemciyiYonetAsync(TcpClient istemci, CancellationToken cancellationToken)
    {
        using (istemci)
        {
            ...
            try
            {
                ...
                if (istek is null) { await MetinCevabiAsync(akis, 400, "Geçersiz istek.", cancellationToken); return; }
    ...
    private async Task YonlendirAsync(NetworkStream akis, HttpIstegi istek, CancellationToken cancellationToken)
    {
        ...
        if (istek.Metot == "GET" && yol == "/favicon.ico") { await CevapAsync(akis, 204, "image/x-icon", string.Empty, null, cancellationToken); return; }
        if (istek.Metot == "GET" && yol == "/api/saglik")
        {
            await JsonCevabiAsync(akis, 200, new { durum = "calisiyor", surum = "yonetim-v5", port = _ayarlar.SirketYonetimPortu, aktifOturum = _oturumlar.Count }, null, cancellationToken);
            return;
        }
        if (istek.Metot == "POST" && yol == "/api/giris")
        {
            GirisIstegi? dto = JsonOku<GirisIstegi>(istek.Govde);
            if (dto is null) { await JsonCevabiAsync(akis, 400, IslemSonucu.Hata("Giriş JSON'u geçersiz."), null, cancellationToken); return; }
            IslemSonucu sonuc = await _isletim.GirisDogrulaAsync(dto, cancellationToken);
            if (!sonuc.Basarili) { await JsonCevabiAsync(akis, 401, sonuc, null, cancellationToken); return; }
            string kimlik = SirketKimliginiOku(sonuc.Veri);
            if (string.IsNullOrWhiteSpace(kimlik)) { await JsonCevabiAsync(akis, 500, IslemSonucu.Hata("Oturum şirketi çözülemedi."), null, cancellationToken); return; }
            string token = Convert.ToHexString(RandomNumberGenerator.GetBytes(32));
            _oturumlar[token] = new OturumKaydi { SirketKimligi = kimlik, SonKullanmaZamani = DateTimeOffset.UtcNow.AddMinutes(OturumDakikasi()) };
            await JsonCevabiAsync(akis, 200, sonuc, new() { ["Set-Cookie"] = $"sirketOturumu={token}; Path=/; HttpOnly; SameSite=Strict" }, cancellationToken);
            return;
        }
        ...
        if (oturum is null)
        {
            await JsonCevabiAsync(akis, 401, IslemSonucu.Hata("Oturum bulunamadı. Yeniden giriş yapın."), new() { ["Set-Cookie"] = "sirketOturumu=; Path=/; Max-Age=0; HttpOnly; SameSite=Strict" }, cancellationToken);
            return;
        }
        ...
        if (istek.Metot == "POST" && yol == "/api/cikis")
        {
            ...
            await JsonCevabiAsync(akis, 200, IslemSonucu.Basari("Çıkış yapıldı."), new() { ["Set-Cookie"] = "sirketOturumu=; Path=/; Max-Age=0; HttpOnly; SameSite=Strict" }, cancellationToken);
            return;
        }
        if (istek.Metot == "GET" && yol == "/api/durum")
        {
            string temel = await _isletim.PanelJsonuOlusturAsync(oturum.SirketKimligi, cancellationToken);
            string zengin = await _ekosistem.PanelJsonunuZenginlestirAsync(oturum.SirketKimligi, temel, cancellationToken);
            await CevapAsync(akis, 200, "application/json; charset=utf-8", zengin, null, cancellationToken);
            return;
        }
        if (istek.Metot != "POST") { await MetinCevabiAsync(akis, 405, "Bu uç POST bekliyor.", cancellationToken); return; }

        IslemSonucu sonucIslem = yol switch
        {
            ...
            "/api/sozlesme/kabul" => await CalistirAsync<SozlesmeKabulIstegi>(istek, x => _isletim.SozlesmeKabulEtAsync(oturum.SirketKimligi, x, cancellationToken)),
            _ => IslemSonucu.Hata("Uç nokta bulunamadı.")
        };
        int kod = sonucIslem.Basarili ? 200 : 400;
        await JsonCevabiAsync(akis, kod, sonucIslem, null, cancellationToken);
    }
    ...
    private OturumKaydi? OturumuGetir(IReadOnlyDictionary<string, string> basliklar)
    {
        ...
        if (kayit.SonKullanmaZamani <= DateTimeOffset.UtcNow) { _oturumlar.TryRemove(token, out _); return null; }
        return kayit;
    }
    ...
    private static T? JsonOku<T>(string json) where T : class { try { return JsonSerializer.Deserialize<T>(json, JsonAyarlari); } catch { return null; } }
    private static string SirketKimliginiOku(object? veri) { try { return JsonSerializer.SerializeToNode(veri, JsonAyarlari)?["sirketKimligi"]?.GetValue<string>() ?? string.Empty; } catch { return string.Empty; } }
    ...
    private static async Task<HttpIstegi?> HttpOkuAsync(NetworkStream akis, CancellationToken cancellationToken)
    {
        ...
        while (bellek.Length < azami)
        {
            ...
            byte[] veri = bellek.GetBuffer();
            baslikSonu = BaslikSonunuBul(veri, (int)bellek.Length);
        ...
        {
            int ikiNokta = satir.IndexOf(':'); if (ikiNokta <= 0) continue;
            basliklar[satir[..ikiNokta].Trim()] = satir[(ikiNokta + 1)..].Trim();
        }
        int uzunluk = 0;
        if (basliklar.TryGetValue("Content-Length", out string? u) && (!int.TryParse(u, out uzunluk) || uzunluk < 0 || uzunluk > azami)) return null;
        int govdeBaslangici = baslikSonu + 4;
        while (bellek.Length - govdeBaslangici < uzunluk && bellek.Length < azami)
        {
            int okunan = await akis.ReadAsync(tampon, cancellationToken); if (okunan <= 0) break; bellek.Write(tampon, 0, okunan);
        }
        tum = bellek.ToArray();
    ...
    {
        for (int i = 0; i <= uzunluk - 4; i++) if (veri[i] == 13 && veri[i + 1] == 10 && veri[i + 2] == 13 && veri[i + 3] == 10) return i;
        return -1;
    }
    private static Task MetinCevabiAsync(NetworkStream akis, int kod, string metin, CancellationToken ct) => CevapAsync(akis, kod, "text/plain; charset=utf-8", metin, null, ct);
    private static Task JsonCevabiAsync(NetworkStream akis, int kod, object veri, Dictionary<string, string>? ek, CancellationToken ct) => CevapAsync(akis, kod, "application/json; charset=utf-8", JsonSerializer.Serialize(veri, JsonAyarlari), ek, ct);
    private static async Task CevapAsync(NetworkStream akis, int kod, string tur, string govde, Dictionary<string, string>? ek, CancellationToken cancellationToken)
    {
        ...
        string durum = kod switch { 200 => "OK", 204 => "No Content", 400 => "Bad Request", 401 => "Unauthorized", 405 => "Method Not Allowed", _ => "Error" };
        StringBuilder b = new();
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        try { _iptal?.Cancel(); _dinleyici?.Stop(); if (_gorev is not null) await _gorev; } catch (OperationCanceledException) { } finally { _iptal?.Dispose(); }
    }
    private sealed class OturumKaydi { public string SirketKimligi { get; init; } = string.Empty; public DateTimeOffset SonKullanmaZamani { get; set; } }
