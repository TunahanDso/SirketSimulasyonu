...
public sealed class SirketYonetimSunucusu : IAsyncDisposable
{
    ...
    public Task BaslatAsync(CancellationToken cancellationToken)
    {
        ...
        KonsolKayitcisi.Basari($"Şirket yönetim merkezi v6 yayında | Port: {_ayarlar.SirketYonetimPortu}");
        return Task.CompletedTask;
    }
    ...
    private async Task YonlendirAsync(NetworkStream akis, HttpIstegi istek, CancellationToken cancellationToken)
    {
        ...
        if (istek.Metot == "GET" && yol == "/api/durum")
        {
            ...
            HashSet<string> sirketPazarlari = pazar.Urunler
                .Where(x => string.Equals(x.SirketKimligi, oturum.SirketKimligi, StringComparison.OrdinalIgnoreCase))
                .Select(x => x.PazarKimligi)
                .ToHashSet(StringComparer.OrdinalIgnoreCase);
            kok["fiyatPazari"] = JsonSerializer.SerializeToNode(new
            {
                pazar.SonTick,
                urunler = pazar.Urunler.Where(x => string.Equals(x.SirketKimligi, oturum.SirketKimligi, StringComparison.OrdinalIgnoreCase)),
                kategoriler = pazar.Kategoriler.Where(x => sirketPazarlari.Contains(x.PazarKimligi)),
                kural = "Fiyat serbesttir; talep ve kullanıcı bağlı değildir. Aşırı fiyat yeni kullanıcıyı durdurur, mevcut kullanıcıyı rakibe kaçırır."
            }, JsonAyarlari);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (ek is not null)
            foreach ((string ad, string deger) in ek) b.Append(ad).Append(": ").Append(deger).Append("\r\n");
        b.Append("\r\n");
