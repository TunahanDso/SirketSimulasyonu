...
public sealed class SirketYonetimSunucusu : IAsyncDisposable
{
    ...
    public Task BaslatAsync(CancellationToken cancellationToken)
    {
        ...
        KonsolKayitcisi.Basari($"Şirket yönetim merkezi V8 yayında | Port: {_ayarlar.SirketYonetimPortu}");
        return Task.CompletedTask;
    }
    ...
    private async Task YonlendirAsync(NetworkStream akis, HttpIstegi istek, CancellationToken cancellationToken)
    {
        ...
        if (istek.Metot == "POST" && yol == "/api/giris")
        {
            ...
            await JsonCevabiAsync(akis, 200, sonuc,
        ...
        if (istek.Metot == "POST" && yol == "/api/cikis")
        {
            ...
            await JsonCevabiAsync(akis, 200, IslemSonucu.Basari("Çıkış yapıldı."),
                new() { ["Set-Cookie"] = "sirketOturumu=; Path=/; Max-Age=0; HttpOnly; SameSite=Strict" }, cancellationToken);
            return;
        }
    ...
    private async Task DurumCevabiAsync(NetworkStream akis, string sirketKimligi, CancellationToken cancellationToken)
    {
        ...
        kok["cezaV8"] = JsonSerializer.SerializeToNode(CezaV8Deposu.SirketOzeti(sirket), JsonAyarlari);
        kok["standartHizmetKatalogu"] = JsonSerializer.SerializeToNode(PanelKatalogV8.HizmetKataloguOlustur(_katalog, sirket), JsonAyarlari);
        kok["uygulamaKategoriKatalogu"] = JsonSerializer.SerializeToNode(PanelKatalogV8.UygulamaKategorileriOlustur(sirket), JsonAyarlari);
