...
public sealed class SirketYonetimSunucusu : IAsyncDisposable
{
    ...
    public Task BaslatAsync(CancellationToken cancellationToken)
    {
        ...
        KonsolKayitcisi.Basari(
            $"Şirket yönetim merkezi V8.2 yayında | Port: {_ayarlar.SirketYonetimPortu}");
        return Task.CompletedTask;
    }
    ...
    private async Task KabulDongusuAsync(CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            {
                KonsolKayitcisi.Uyari($"8090 istemci kabul hatası: {hata.Message}");
        ...
        CancellationToken cancellationToken)
    {
        using (istemci)
        {
            ...
            {
                KonsolKayitcisi.Uyari($"8090 istemci hatası: {hata.Message}");
            }
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (istek.Metot == "GET" && yol == "/api/saglik")
        {
            await JsonCevabiAsync(akis, 200, new
            {
                ...
                katalogHizmeti = _katalog.Hizmetler.Count,
                uygulamaKategorisi = StandartKatalogV6.UygulamaKategorileri.Count,
                hizmetFiyati = "motor-sabit",
        ...
        if (istek.Metot == "POST" && yol == "/api/giris")
        {
            ...
            if (dto is null)
            {
                await JsonCevabiAsync(
                    akis,
                    400,
                    IslemSonucu.Hata("Giriş JSON'u geçersiz."),
                    null,
                    cancellationToken);
                return;
            }
            ...
            if (string.IsNullOrWhiteSpace(kimlik))
            {
                await JsonCevabiAsync(
                    akis,
                    500,
                    IslemSonucu.Hata("Oturum şirketi çözülemedi."),
                    null,
                    cancellationToken);
                return;
            }
            ...
            await JsonCevabiAsync(
                ...
                200,
                sonuc,
                new() { ["Set-Cookie"] = $"sirketOturumu={token}; Path=/; HttpOnly; SameSite=Strict" },
                cancellationToken);
            return;
        }
        ...
        if (yol == "/api/sozlesme/kabul")
        {
            ...
            if (dto is null)
                sonucIslem = IslemSonucu.Hata("SLA kabul isteği geçersiz.");
            else if (!_finans.SlaKabulEdilebilirMi(
                         oturum.SirketKimligi,
                         dto.TeklifKimligi,
                         out string neden))
                sonucIslem = IslemSonucu.Hata(neden);
            else
                sonucIslem = await _isletim.SozlesmeKabulEtAsync(
                    oturum.SirketKimligi,
                    dto,
                    cancellationToken);
        }
        else if (yol == "/api/uygulama/yayinla")
        {
            UygulamaYayinlaIstegi? dto = JsonOku<UygulamaYayinlaIstegi>(istek.Govde);
            sonucIslem = dto is null
                ? IslemSonucu.Hata("Uygulama yayın isteği geçersiz.")
                : await UygulamaYayinlaVeDagitAsync(
                    oturum.SirketKimligi,
                    dto,
                    cancellationToken);
        }
        else
        {
            ...
            {
                "/api/parola" => await CalistirAsync<ParolaDegistirIstegi>(
                    istek,
                    x => _isletim.ParolaDegistirAsync(oturum.SirketKimligi, x, cancellationToken)),
                "/api/yatirim" => await CalistirAsync<YatirimIstegi>(
                    istek,
                    x => _isletim.YatirimSatinAlAsync(oturum.SirketKimligi, x, cancellationToken)),
                "/api/kredi" => await CalistirAsync<KrediIstegi>(
                    istek,
                    x => _isletim.KrediCekAsync(oturum.SirketKimligi, x, cancellationToken)),
                "/api/hizmet/fiyat" => IslemSonucu.Hata(
                    "Hizmet fiyatları motor kataloğunda sabittir; yalnız uygulama ve işletim sistemi fiyatları yönetilebilir."),
                "/api/hizmet/kapasite" => IslemSonucu.Hata(
                    "Hizmet gücü satın alınamaz. Gerçek kapasite yalnız altyapı yatırımlarından oluşur."),
                "/api/hizmet/durum" => await CalistirAsync<HizmetYayinDurumuIstegi>(
                    istek,
                    x => _isletim.HizmetYayinDurumuGuncelleAsync(oturum.SirketKimligi, x, cancellationToken)),
                "/api/uygulama/guncelle" => await CalistirAsync<UrunGuncelleIstegi>(
                    istek,
                    x => _isletim.UygulamaGuncelleAsync(oturum.SirketKimligi, x, cancellationToken)),
                "/api/uygulama/kapasite" => IslemSonucu.Hata(
                    "Uygulama kapasitesi ayrıca satın alınamaz. Kullanıcı sınırı fiziksel altyapı ve yatırımlardan hesaplanır."),
                "/api/uygulama/dagitim" => await CalistirAsync<UrunDagitimGuncelleIstegi>(
                    istek,
                    x => _ekosistem.DagitimGuncelleAsync(oturum.SirketKimligi, x, cancellationToken)),
                "/api/protokol/yayinla" => await CalistirAsync<ProtokolYayinlaIstegi>(
                    istek,
                    x => _isletim.ProtokolYayinlaAsync(oturum.SirketKimligi, x, cancellationToken)),
                "/api/protokol/benimse" => await CalistirAsync<ProtokolBenimseIstegi>(
                    istek,
                    x => _isletim.ProtokolBenimseAsync(oturum.SirketKimligi, x, cancellationToken)),
        ...
        await JsonCevabiAsync(
            akis,
            sonucIslem.Basarili ? 200 : 400,
            sonucIslem,
            null,
            cancellationToken);
    }

    private async Task<IslemSonucu> UygulamaYayinlaVeDagitAsync(
        string sirketKimligi,
        UygulamaYayinlaIstegi istek,
        CancellationToken cancellationToken)
    {
        SunulanUygulama? manifest = SunucuYayinManifestDeposu.Getir(sirketKimligi)
            .Uygulamalar.FirstOrDefault(x => x.UygulamaKimligi.Equals(
                istek.UygulamaKimligi?.Trim(),
                StringComparison.OrdinalIgnoreCase));
        if (manifest is null)
            return IslemSonucu.Hata("Uygulama şirket sunucusunun manifestinde bulunamadı.");

        bool isletimSistemi = manifest.UrunTuru.Equals(
            "isletim-sistemi",
            StringComparison.OrdinalIgnoreCase);
        if (string.IsNullOrWhiteSpace(istek.BaglantiProtokoluKimligi))
            return IslemSonucu.Hata("Yayın için bağlantı protokolü seçilmelidir.");
        if (!isletimSistemi && string.IsNullOrWhiteSpace(istek.IsletimSistemiKimligi))
            return IslemSonucu.Hata("Uygulama için işletim sistemi seçilmelidir.");

        FiyatModeliniNormalizeEt(istek);
        IslemSonucu yayin = await _isletim.UygulamaYayinlaAsync(
            sirketKimligi,
            istek,
            cancellationToken);
        if (!yayin.Basarili) return yayin;

        string urunKimligi = VeriAlani(yayin.Veri, "urunKimligi");
        if (string.IsNullOrWhiteSpace(urunKimligi))
            return IslemSonucu.Hata("Ürün oluştu fakat dağıtım kimliği alınamadı.");

        IslemSonucu dagitim = await _ekosistem.DagitimGuncelleAsync(
            sirketKimligi,
            new UrunDagitimGuncelleIstegi
            {
                UrunKimligi = urunKimligi,
                IsletimSistemiKimligi = isletimSistemi
                    ? string.Empty
                    : istek.IsletimSistemiKimligi,
        ...
        if (!dagitim.Basarili)
        {
            await _isletim.UygulamaGuncelleAsync(
                sirketKimligi,
            ...
            return IslemSonucu.Hata(
                "Ürün kaydedildi fakat dağıtım uyumsuz olduğu için pasif bırakıldı: " +
                dagitim.Aciklama);
        }

        return IslemSonucu.Basari(
            "Uygulama, işletim sistemi ve protokol bağlantısıyla tek işlemde yayına alındı.",
            new { urunKimligi, dagitim = dagitim.Veri });
    }

    private async Task DurumCevabiAsync(
        NetworkStream akis,
        string sirketKimligi,
        CancellationToken cancellationToken)
    {
        MotorHizmetFiyatlari.Uygula(_sirketler.SirketKayitlari);
        string temel = await _isletim.PanelJsonuOlusturAsync(
            sirketKimligi,
            cancellationToken);
        string zengin = await _ekosistem.PanelJsonunuZenginlestirAsync(
            sirketKimligi,
            temel,
            cancellationToken);
        JsonObject kok = JsonNode.Parse(zengin) as JsonObject ?? new();
        SirketKaydi sirket = _sirketler.SirketKayitlari.First(x =>
        ...
        HashSet<string> sirketPazarlari = pazar.Urunler
            .Where(x => x.SirketKimligi.Equals(sirketKimligi, StringComparison.OrdinalIgnoreCase))
            .Select(x => x.PazarKimligi)
            .ToHashSet(StringComparer.OrdinalIgnoreCase);
        kok["fiyatPazari"] = JsonSerializer.SerializeToNode(new
        {
            pazar.SonTick,
            urunler = pazar.Urunler.Where(x => x.SirketKimligi.Equals(sirketKimligi, StringComparison.OrdinalIgnoreCase)),
            kategoriler = pazar.Kategoriler.Where(x => sirketPazarlari.Contains(x.PazarKimligi)),
            kural = "Yalnız uygulama ve işletim sistemi fiyatları serbesttir; talep aşırı fiyatı cezalandırır."
        }, JsonAyarlari);
        ...
        IEnumerable<object> haberler = FinansV7Deposu.Getir().Haberler
            .Where(x => string.IsNullOrWhiteSpace(x.SirketKimligi) ||
                        x.SirketKimligi.Equals(sirketKimligi, StringComparison.OrdinalIgnoreCase))
            .OrderByDescending(x => x.TickNumarasi).Take(40).Cast<object>()
            .Concat(v6.Haberler
                .Where(x => string.IsNullOrWhiteSpace(x.SirketKimligi) ||
                            x.SirketKimligi.Equals(sirketKimligi, StringComparison.OrdinalIgnoreCase))
        ...
        kok["bankaV7"] = JsonSerializer.SerializeToNode(
            _finans.BankaOzetiGetir(sirketKimligi),
            JsonAyarlari);
        kok["slaUygunluklari"] = JsonSerializer.SerializeToNode(
            _finans.SlaUygunluklariniGetir(sirketKimligi),
            JsonAyarlari);
        kok["haberBulteni"] = JsonSerializer.SerializeToNode(haberler, JsonAyarlari);
        kok["v6Sirket"] = JsonSerializer.SerializeToNode(
            v6.Sirketler.FirstOrDefault(x =>
                x.SirketKimligi.Equals(sirketKimligi, StringComparison.OrdinalIgnoreCase)),
            JsonAyarlari);
        kok["cezaV8"] = JsonSerializer.SerializeToNode(
            CezaV8Deposu.SirketOzeti(sirket),
            JsonAyarlari);
        kok["standartHizmetKatalogu"] = JsonSerializer.SerializeToNode(
            PanelKatalogV8.HizmetKataloguOlustur(_katalog, sirket),
            JsonAyarlari);
        kok["uygulamaKategoriKatalogu"] = JsonSerializer.SerializeToNode(
            PanelKatalogV8.UygulamaKategorileriOlustur(sirket),
            JsonAyarlari);
        kok["protokolKataloguV82"] = JsonSerializer.SerializeToNode(
            ProtokolKataloguOlustur(sirketKimligi, kok),
            JsonAyarlari);
        kok["hizmetFiyatKurali"] = JsonValue.Create(
            "Hizmet fiyatları motor tarafından sabittir ve hiçbir şirket tarafından değiştirilemez.");

        await CevapAsync(
            akis,
            200,
            "application/json; charset=utf-8",
            kok.ToJsonString(JsonAyarlari),
            null,
            cancellationToken);
    }

    private IReadOnlyList<object> ProtokolKataloguOlustur(
        string oturumSirketi,
        JsonObject panel)
    {
        List<OzelProtokolKaydi> piyasa;
        try
        {
            piyasa = panel["protokoller"]?.Deserialize<List<OzelProtokolKaydi>>(JsonAyarlari) ?? [];
        }
        catch
        {
            piyasa = [];
        ...
        List<object> sonuc = [];
        HashSet<string> eslesenPiyasa = new(StringComparer.OrdinalIgnoreCase);
        foreach (SunucuYayinManifesti manifest in SunucuYayinManifestDeposu.TumunuGetir())
        {
            string sahipAdi = _sirketler.SirketKayitlari.FirstOrDefault(x =>
                x.SirketKimligi.Equals(manifest.SirketKimligi, StringComparison.OrdinalIgnoreCase))?.SirketAdi
                ?? manifest.SirketKimligi;
            foreach (SunulanOzelProtokol teknik in manifest.OzelProtokoller)
            {
                ...
                if (kayit is not null) eslesenPiyasa.Add(kayit.ProtokolKimligi);
                bool sahibi = manifest.SirketKimligi.Equals(
                    oturumSirketi,
                    StringComparison.OrdinalIgnoreCase);
                bool benimsendi = sahibi ||
                    kayit?.BenimseyenSirketler.Contains(
                        oturumSirketi,
                        StringComparer.OrdinalIgnoreCase) == true;
                bool acik = string.Equals(
                    kayit?.LisansModeli,
                    "acik",
                    StringComparison.OrdinalIgnoreCase);
                sonuc.Add(new
                {
                    protokolKimligi = teknik.ProtokolKimligi,
                    piyasaKaydiKimligi = kayit?.ProtokolKimligi ?? string.Empty,
                    teknik.ProtokolAdi,
                    teknik.Surum,
                    teknik.Aciklama,
                    sahipSirketKimligi = manifest.SirketKimligi,
                    sahipSirketAdi = sahipAdi,
                    aktif = kayit?.Aktif ?? false,
                    sahibi,
                    benimsendi,
                    kullanilabilir = kayit?.Aktif == true && (sahibi || benimsendi || acik),
                    lisansModeli = kayit?.LisansModeli ?? "yayinlanmadi",
                    benimsemeBedeli = kayit?.BenimsemeBedeli ?? 0,
                    tickLisansBedeli = kayit?.TickLisansBedeli ?? 0,
                    yetkinlikSayisi = teknik.Yetkinlikler.Count
                });
            }
        }

        foreach (OzelProtokolKaydi kayit in piyasa.Where(x =>
                     !eslesenPiyasa.Contains(x.ProtokolKimligi)))
        {
            bool sahibi = kayit.SahipSirketKimligi.Equals(
                oturumSirketi,
                StringComparison.OrdinalIgnoreCase);
            bool benimsendi = sahibi || kayit.BenimseyenSirketler.Contains(
                oturumSirketi,
                StringComparer.OrdinalIgnoreCase);
            sonuc.Add(new
            {
                protokolKimligi = kayit.ProtokolKimligi,
                piyasaKaydiKimligi = kayit.ProtokolKimligi,
                kayit.ProtokolAdi,
                kayit.Surum,
                kayit.Aciklama,
                kayit.SahipSirketKimligi,
                sahipSirketAdi = kayit.SahipSirketKimligi,
                kayit.Aktif,
                sahibi,
                benimsendi,
                kullanilabilir = kayit.Aktif &&
                    (sahibi || benimsendi || kayit.LisansModeli.Equals("acik", StringComparison.OrdinalIgnoreCase)),
                kayit.LisansModeli,
                kayit.BenimsemeBedeli,
                kayit.TickLisansBedeli,
                yetkinlikSayisi = 0
            });
        }

        return sonuc
            .OrderBy(x => JsonSerializer.SerializeToNode(x)?["protokolAdi"]?.GetValue<string>(),
                StringComparer.OrdinalIgnoreCase)
            .ToList()
            .AsReadOnly();
    }

    private static void FiyatModeliniNormalizeEt(UygulamaYayinlaIstegi istek)
    {
        string model = istek.FiyatlandirmaModeli?.Trim().ToLowerInvariant() ?? "abonelik";
        switch (model)
        {
            case "kullanim":
                istek.AbonelikUcreti = 0;
                break;
            case "abonelik":
            case "lisans":
            case "tek-seferlik":
                istek.KullanimBasinaUcret = 0;
                break;
            case "freemium":
                break;
            default:
                istek.FiyatlandirmaModeli = "abonelik";
                istek.KullanimBasinaUcret = 0;
                break;
        }
    }
    ...
    private OturumKaydi? OturumuGetir(IReadOnlyDictionary<string, string> basliklar)
    {
        ...
        if (string.IsNullOrWhiteSpace(token) ||
            !_oturumlar.TryGetValue(token, out OturumKaydi? kayit))
            return null;
    ...
    private static string SirketKimliginiOku(object? veri) =>
        VeriAlani(veri, "sirketKimligi");

    private static string VeriAlani(object? veri, string alan)
    {
        try
        {
            return JsonSerializer.SerializeToNode(veri, JsonAyarlari)?[alan]?.GetValue<string>()
                   ?? string.Empty;
        }
        catch
        {
        ...
        string ad)
    {
        ...
        foreach (string parca in cookie.Split(';', StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            if (ikili.Length == 2 && string.Equals(ikili[0], ad, StringComparison.Ordinal))
                return ikili[1];
        }
        return null;
    }

    private static async Task<HttpIstegi?> HttpOkuAsync(
        NetworkStream akis,
        CancellationToken cancellationToken)
    {
        ...
        if (basliklar.TryGetValue("Content-Length", out string? u) &&
            (!int.TryParse(u, out uzunluk) || uzunluk < 0 || uzunluk > azami))
            return null;
        ...
        string govde = uzunluk == 0
            ? string.Empty
            : Encoding.UTF8.GetString(tum, govdeBaslangici, uzunluk);
        return new HttpIstegi(
            ilk[0].ToUpperInvariant(),
            ilk[1],
            basliklar,
            govde);
    }
    ...
    private static int BaslikSonunuBul(byte[] veri, int uzunluk)
    {
        ...
        {
            if (veri[i] == 13 && veri[i + 1] == 10 &&
                veri[i + 2] == 13 && veri[i + 3] == 10)
                return i;
        }
        return -1;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (ek is not null)
        {
            foreach ((string ad, string deger) in ek)
                baslik.Append(ad).Append(": ").Append(deger).Append("\r\n");
        }
        baslik.Append("\r\n");
        await akis.WriteAsync(Encoding.ASCII.GetBytes(baslik.ToString()), cancellationToken);
