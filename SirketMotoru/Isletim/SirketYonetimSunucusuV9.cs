...
public sealed class V92TopluKapasiteTahsisIstegi
{
    public int HizmetHavuzu { get; set; }
    public Dictionary<string, int> UrunTahsisleri { get; set; } = new(StringComparer.OrdinalIgnoreCase);
...
public sealed class SirketYonetimSunucusu : IAsyncDisposable
{
    ...
    public Task BaslatAsync(CancellationToken cancellationToken)
    {
        ...
        KonsolKayitcisi.Basari($"Şirket yönetim merkezi V9.2 yayında | Port: {_ayarlar.SirketYonetimPortu}");
    ...
    private async Task YonlendirAsync(NetworkStream akis, HttpIstegi istek, CancellationToken ct)
    {
        ...
        {
            await JsonAsync(akis, 200, new
            {
                durum = "calisiyor",
                surum = "yonetim-v9.2",
                fiyat = "kategori-sinirli",
                kapasite = "tek-fiziksel-havuz-toplu-hizmet",
                protokol = "coklu"
            }, null, ct); return;
        }
        if (istek.Metot == "GET" && yol == "/favicon.ico")
        {
            await CevapAsync(akis, 204, "image/x-icon", "", null, ct); return;
        }
        ...
        if (istek.Metot == "POST" && yol == "/api/giris")
        {
            ...
            {
                SirketKimligi = sirket,
                SonKullanma = DateTimeOffset.UtcNow.AddMinutes(OturumDakikasi())
            };
            await JsonAsync(akis, 200, sonuc, new()
            {
        ...
        if (oturum is null)
        {
            await JsonAsync(akis, 401, IslemSonucu.Hata("Oturum bulunamadı."), new()
            {
                ["Set-Cookie"] = "sirketOturumu=; Path=/; Max-Age=0; HttpOnly"
            }, ct); return;
        }
        ...
        if (istek.Metot == "POST" && yol == "/api/cikis")
        {
            ...
            await JsonAsync(akis, 200, IslemSonucu.Basari("Çıkış yapıldı."), new()
            {
                ["Set-Cookie"] = "sirketOturumu=; Path=/; Max-Age=0; HttpOnly"
            }, ct); return;
        }
        ...
        switch (yol)
        {
            ...
            case "/api/kapasite/tahsis":
                V92TopluKapasiteTahsisIstegi? kapasite = Oku<V92TopluKapasiteTahsisIstegi>(istek.Govde);
                islem = kapasite is null
                    ? IslemSonucu.Hata("Kapasite isteği geçersiz.")
                    : await TopluKapasiteTahsisEtAsync(oturum.SirketKimligi, kapasite, ct);
                break;
            ...
            case "/api/uygulama/yayinla":
                ...
                islem = yayin is null
                    ? IslemSonucu.Hata("Yayın isteği geçersiz.")
                    : await UygulamaYayinlaAsync(oturum.SirketKimligi, yayin, ct);
                break;
    ...
    private async Task<IslemSonucu> TopluKapasiteTahsisEtAsync(
        string sirketKimligi,
        V92TopluKapasiteTahsisIstegi istek,
        CancellationToken ct)
    {
        ...
        if (sirket is null) return IslemSonucu.Hata("Şirket bulunamadı.");

        // Eski V9.1 istemcisi doğrudan ayrıntılı tahsis göndermişse çalışmaya devam eder.
        if (istek.Tahsisler.Count > 0)
            return await _v9.KapasiteTahsisEtAsync(sirketKimligi, new V9KapasiteTahsisIstegi
            {
                Tahsisler = new Dictionary<string, int>(istek.Tahsisler, StringComparer.OrdinalIgnoreCase)
            }, ct);

        Dictionary<string, int> tahsisler = new(StringComparer.OrdinalIgnoreCase);
        List<SunulanHizmet> hizmetler = sirket.Hizmetler.Where(x => x.Aktif).ToList();
        int hizmetHavuzu = Math.Max(0, istek.HizmetHavuzu);
        if (hizmetler.Count > 0 && hizmetHavuzu > 0)
        {
            V9PazarDosyasi pazar = V9PazarDeposu.Getir();
            Dictionary<SunulanHizmet, double> agirliklar = hizmetler.ToDictionary(
                h => h,
                h => Math.Max(1,
                    pazar.HizmetTalepleri.TryGetValue($"{h.HizmetKimligi}@{h.HizmetSurumu}", out V9TalepKaydi? talep)
                        ? talep.BuTickTalep
                        : 1));
            ...
            {
                SunulanHizmet h = hizmetler[i];
                int pay = i == hizmetler.Count - 1
                    ? Math.Max(0, hizmetHavuzu - kullanilan)
                    : Math.Max(0, (int)Math.Floor(hizmetHavuzu * agirliklar[h] / Math.Max(1, toplamAgirlik)));
                tahsisler[$"hizmet:{h.HizmetKimligi}@{h.HizmetSurumu}"] = pay;
                kullanilan += pay;
            }
        }

        foreach ((string anahtar, int deger) in istek.UrunTahsisleri)
        {
            string temiz = anahtar.StartsWith("urun:", StringComparison.OrdinalIgnoreCase)
                ? anahtar
                : $"urun:{anahtar}";
            tahsisler[temiz] = Math.Max(0, deger);
        }
    ...
    private async Task<IslemSonucu> UygulamaYayinlaAsync(
        string sirketKimligi,
        ...
        CancellationToken ct)
    {
        ...
        if (!os && string.IsNullOrWhiteSpace(istek.IsletimSistemiKimligi))
            return IslemSonucu.Hata("İşletim sistemi seçin.");
        IReadOnlyList<string> protokoller = istek.ProtokolleriGetir();
        if (protokoller.Count == 0) return IslemSonucu.Hata("En az bir protokol seçin.");

        decimal fiyat = V9FiyatPolitikasi.Sinirla(
            manifest.UrunTuru,
            manifest.Kategori,
            istek.FiyatlandirmaModeli == "kullanim" ? istek.KullanimBasinaUcret : istek.AbonelikUcreti);
        if (istek.FiyatlandirmaModeli == "kullanim")
        {
            istek.KullanimBasinaUcret = fiyat;
            istek.AbonelikUcreti = 0;
        }
        else
        {
            istek.AbonelikUcreti = fiyat;
            if (istek.FiyatlandirmaModeli != "freemium") istek.KullanimBasinaUcret = 0;
        }
        istek.BaglantiProtokoluKimlikleri = protokoller.ToList();
        istek.BaglantiProtokoluKimligi = protokoller[0];
        ...
        IslemSonucu dagitim = await _ekosistem.DagitimGuncelleAsync(sirketKimligi, new UrunDagitimGuncelleIstegi
        {
            ...
            BaglantiProtokoluKimlikleri = protokoller.ToList(),
            BaglantiProtokoluKimligi = protokoller[0],
        ...
        return IslemSonucu.Basari(
            $"Ürün OS ve {protokoller.Count} kanonik protokolle yayına alındı.",
            new { urunKimligi, fiyat, protokoller });
    }
    ...
    private async Task DurumAsync(NetworkStream akis, string sirketKimligi, CancellationToken ct)
    {
        ...
        kok["v9"] = JsonSerializer.SerializeToNode(new
        {
            ...
            haberler = pazar.Haberler.OrderByDescending(x => x.TickNumarasi).Take(150),
            fiyatKurali = "Uygulama ve OS fiyatları kategori min-max aralığında; hizmet fiyatları sabit.",
            kapasiteKurali = "Tek fiziksel havuz: hizmetler toplu pay, her uygulama ve OS ayrı pay alır."
        }, JsonAyarlari);
    ...
    private static string Alan(object? veri, string alan)
    {
        try { return JsonSerializer.SerializeToNode(veri, JsonAyarlari)?[alan]?.GetValue<string>() ?? ""; }
        catch { return ""; }
    }
    ...
    private static string? Cookie(IReadOnlyDictionary<string, string> basliklar, string ad)
    {
        ...
        foreach (string p in c.Split(';', StringSplitOptions.RemoveEmptyEntries))
        {
            string[] x = p.Trim().Split('=', 2);
            if (x.Length == 2 && x[0] == ad) return x[1];
        }
        return null;
    }
    ...
    private static async Task<HttpIstegi?> HttpOkuAsync(NetworkStream akis, CancellationToken ct)
    {
        ...
        {
            int n = await akis.ReadAsync(tampon, ct);
            if (n <= 0) break;
            bellek.Write(tampon, 0, n);
            son = BaslikSonu(bellek.GetBuffer(), (int)bellek.Length);
            if (son >= 0) break;
        }
        if (son < 0) return null;
        byte[] tum = bellek.ToArray();
        string[] satir = Encoding.ASCII.GetString(tum, 0, son).Split("\r\n");
        string[] ilk = satir[0].Split(' ', 3, StringSplitOptions.RemoveEmptyEntries);
        if (ilk.Length < 2) return null;
        Dictionary<string, string> basliklar = new(StringComparer.OrdinalIgnoreCase);
        foreach (string s in satir.Skip(1))
        {
            int i = s.IndexOf(':');
            if (i > 0) basliklar[s[..i].Trim()] = s[(i + 1)..].Trim();
        }
        ...
        int bas = son + 4;
        while (bellek.Length - bas < uzunluk)
        {
            int n = await akis.ReadAsync(tampon, ct);
            if (n <= 0) break;
            bellek.Write(tampon, 0, n);
        ...
        tum = bellek.ToArray();
        if (tum.Length - bas < uzunluk) return null;
        return new HttpIstegi(
            ilk[0].ToUpperInvariant(),
            ilk[1],
            basliklar,
            uzunluk == 0 ? "" : Encoding.UTF8.GetString(tum, bas, uzunluk));
    }

    private static int BaslikSonu(byte[] b, int n)
    {
        for (int i = 0; i <= n - 4; i++)
    ...
    private static Task MetinAsync(NetworkStream a, int k, string m, CancellationToken ct) =>
        CevapAsync(a, k, "text/plain; charset=utf-8", m, null, ct);

    private static Task JsonAsync(NetworkStream a, int k, object v, Dictionary<string, string>? e, CancellationToken ct) =>
        CevapAsync(a, k, "application/json; charset=utf-8", JsonSerializer.Serialize(v, JsonAyarlari), e, ct);
    ...
    private static async Task CevapAsync(
        ...
        CancellationToken ct)
    {
        byte[] b = Encoding.UTF8.GetBytes(govde);
        ...
        if (ek is not null)
            foreach ((string a, string d) in ek) h.Append(a).Append(": ").Append(d).Append("\r\n");
        h.Append("\r\n");
        await akis.WriteAsync(Encoding.ASCII.GetBytes(h.ToString()), ct);
        if (b.Length > 0) await akis.WriteAsync(b, ct);
    ...
    private sealed class OturumKaydi
    {
        ...
        public DateTimeOffset SonKullanma { get; set; }
    }
