...
public sealed class TickYoneticisi
{
        ...
        PazarGelirDuzeltmeYoneticisi pazarGelirDuzeltmeYoneticisi)
    {
        ...
        if (ayarlar.TickSuresiSaniye <= 0)
            throw new ArgumentOutOfRangeException(nameof(ayarlar));
    ...
    {
        if (_calisiyor)
            throw new InvalidOperationException("Tick sistemi zaten çalışıyor.");
        _calisiyor = true;
        KonsolKayitcisi.Bilgi(
            $"Tick sistemi başlatıldı. Tick süresi: {_ayarlar.TickSuresiSaniye} saniye.");
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                catch (Exception e)
                {
                    KonsolKayitcisi.Hata(
                        $"Tick {tick} çalıştırılırken hata oluştu: {e.Message}");
                ...
                KonsolKayitcisi.Bilgi(
                    $"Tick {tick} tamamlandı. İşlem süresi: " +
                ...
                if (bekleme <= TimeSpan.Zero)
                {
                    KonsolKayitcisi.Uyari(
                        $"Tick {tick} hedef süreden uzun sürdü; " +
                        "sonraki tick beklemeden başlayacak.");
                    continue;
                }
                try
                {
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Ürün ekonomisinden önce gelir ve kullanıcı sayıları kaydedilir.
        // Sonra temel ekonomi çalışır; fiyat pazarı kullanıcı kaçışı ve göçü uygular;
        // en sonda pazar dışı fiyatlardan yazılan hayalî gelir chargeback ile geri alınır.
        ...
        IReadOnlyList<HizmetTalebi> talepler)
    {
        ...
        foreach (IGrouping<string, HizmetTalebi> grup in talepler
                     .GroupBy(
                         t => $"{t.HizmetKimligi}@{t.HizmetSurumu}",
                     ...
                     .ThenBy(g => g.Key))
        {
            KonsolKayitcisi.Bilgi(
                ...
                $"Bütçe: {grup.Sum(t => t.AzamiButce):N2}");
        }
    }
        ...
        IReadOnlyList<HizmetTalebi> talepler)
    {
        ...
        int osBekleyen = Math.Max(
            0,
            _musteriYoneticisi.AktifMusteriSayisi - osKullanan);
        int fiyatKaybi = fiyatPazari.Urunler.Sum(
            x => x.BuTickFiyatKaybi + x.BuTickEngellenenYeniKullanici);
        int rakibeGoc = fiyatPazari.Urunler.Sum(
            x => x.BuTickRakiptenGelenKullanici);

        KonsolKayitcisi.Bilgi(
            $"Tick {tickNumarasi} özeti | " +
            $"Aktif müşteri: {_musteriYoneticisi.AktifMusteriSayisi} | " +
            $"OS kullanan: {osKullanan} | OS bekleyen: {osBekleyen} | " +
            $"Aktif OS: {sistemler.Count} | " +
            $"Fiyat nedeniyle reddedilen/kaçan: {fiyatKaybi} | " +
            $"Rakibe göç: {rakibeGoc} | " +
            $"Yeni talep: {talepler.Count} | " +
            $"Talep bütçesi: {talepler.Sum(t => t.AzamiButce):N2} | " +
    ...
    private async Task SonKayitlariYapAsync()
    {
        ...
        {
            KonsolKayitcisi.Hata(
                $"Son kayıtlar yazılamadı: {e.Message}");
        }
    }
}
