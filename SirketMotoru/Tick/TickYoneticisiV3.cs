...
public sealed class TickYoneticisi
{
    ...
    private readonly BaglantiBazliSirketDonmaYoneticisi _donmaYoneticisi;
        ...
        FinansV7Yoneticisi finansV7Yoneticisi)
    {
        ...
        _donmaYoneticisi = new BaglantiBazliSirketDonmaYoneticisi(
            sirketYoneticisi,
            isletimYoneticisi,
            finansV7Yoneticisi);
    ...
    public async Task BaslatAsync(CancellationToken cancellationToken)
    {
        ...
        KonsolKayitcisi.Bilgi(
            ...
            $"Tick süresi: {_ayarlar.TickSuresiSaniye} saniye | Bağlı şirketler oynar, kapalı şirketler donar.");
        ...
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                {
                    KonsolKayitcisi.Hata($"Tick {tick} tamamlanamadı; tick saati ilerletilmedi: {e.Message}");
    ...
    private async Task TickCalistirAsync(long tickNumarasi, CancellationToken cancellationToken)
    {
        ...
        await _donmaYoneticisi.TickOncesiAsync(cancellationToken);
        ...
        try
        {
            ...
            // Kapalı şirketlere eski katmanların yazdığı her şey tick kapanmadan geri alınır.
            await _donmaYoneticisi.GeriYukleAsync(tickNumarasi, cancellationToken);
        ...
        {
            // Hata veya iptalde kapalı şirketlerin anlık görüntüsü mutlaka geri yüklenir.
            await _donmaYoneticisi.GeriYukleAsync(tickNumarasi, CancellationToken.None);
        }
    }
    ...
    private async Task SonKayitlariYapAsync()
    {
        ...
        {
            KonsolKayitcisi.Hata($"Son kayıtlar yazılamadı: {e.Message}");
        }
    }
}
