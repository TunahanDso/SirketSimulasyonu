...
fn akis_getir(v: &Map<String, Value>) -> Result<Value, HizmetHatasi> {
    ...
    let result = s.gonderiler.iter().rev().take(count).map(|p| {
        ...
        let post_id = post.get("gonderiKimligi").and_then(Value::as_str).map(str::to_string);
        if let Some(post_id) = post_id {
            let summary = etkilesim_ozeti(s.etkilesimler.get(&post_id));
            post["etkilesimler"] = json!(summary);
        }
        post
    }).collect::<Vec<_>>();
    Ok(json!(result))
...
fn ek_yukle(v: &Map<String, Value>) -> Result<Value, HizmetHatasi> {
    let size = v.get("boyutByte").and_then(Value::as_u64)
        .ok_or_else(|| HizmetHatasi::GecersizIstek("boyutByte zorunludur.".to_string()))?;
    if size > 25_000_000 {
        return Err(HizmetHatasi::GecersizIstek("Ek 25 MB sınırını aşıyor.".to_string()));
    }
    let attachment = json!({
...
fn tlink_dogrula(v: &Map<String, Value>) -> Result<Value, HizmetHatasi> {
    let packet = v.get("paket").and_then(Value::as_object)
